...
# CSV file setup
csv_filename = "experiment_1a_results.csv"
with open(csv_filename, "w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Time", "Distance to Obstacle", "Distance to Goal"])
...
def step(t):
    ...
    with open(csv_filename, "a", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([t, np.linalg.norm(Te.t - s0.T[:3, -1]), np.linalg.norm(Te.t - Tep.t)])
