...
# Step function with moving target
def step(time):
    update_obstacles(time)

    # Move target across the table at 0.1 m/s for 4 seconds
    if time <= 4:
        Tep.A[0, 3] += 0.1 * 0.01  # Move along x-axis
    ...
    # Store data
    data.append([time, neo_distance_to_goal, neo_distance_to_obstacle1, neo_distance_to_obstacle2])

    env.step(0.01)
    
    # Stop when the robot is close enough to the target
    return neo_distance_to_goal < 0.02  

# Run experiment with a time limit
def run():
    arrived = False
    time = 0
    ...
    df = pd.DataFrame(data, columns=["time", "neo_distance_to_goal", "neo_distance_to_obstacle1", "neo_distance_to_obstacle2"])
